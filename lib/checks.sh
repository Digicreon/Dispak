...
check_platform() {
	...
	echo "$HOSTNAME" | grep -Eq "^test[0-9]*$|^staging[0-9]*$|^stage[0-9]*$|^qualification[0-9]*$|qualif[0-9]*$|^quality[0-9]*$|^qa[0-9]*$|^preprod[0-9]*$|^pprod[0-9]*$"
	if [ $? -eq 0 ]; then
		DPK_OPT["platform"]="test"
	else
		echo "$HOSTNAME" | grep -Eq "^server[0-9]*$|^serv[0-9]*$|^srv[0-9]*$|^production[0-9]*$|^prod[0-9]*$|^administration[0-9]*$|^admin[0-9]*$|^web[0-9]*$|^www[0-9]*$|^$|^database[0-9]*$|^data[0-9]*$|^db[0-9]*$|^cron[0-9]*$|^worker[0-9]*$|^jobs[0-9]*$|^frontend[0-9]*$|^front[0-9]*$|^backend[0-9]*$|^back[0-9]*$"
		if [ $? -eq 0 ]; then
