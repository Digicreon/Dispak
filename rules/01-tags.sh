...
rule_exec_tags() {
	local LAST_MAJOR LAST_MINOR FIRST_REVISION LAST_REVISION LAST_DATE SHOWN TAGS TAG TAG_DATE TAG_MAJOR TAG_MINOR TAG_REVISION TAGGER LINE LEN SPACES NBR_COMMITS
	check_git
	LAST_MAJOR=""
	LAST_MINOR=""
	FIRST_REVISION=""
	...
	for TAG in $TAGS; do
		...
		if [ "$TAG_MINOR" = "0" ] && [ "$TAG_REVISION" = "0" ]; then
			...
			if [ -v DPK_OPT["all"] ] || [ "$FIRST_REVISION" = "" ]; then
