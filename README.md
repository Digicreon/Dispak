...
- **pkg rule**
  ...
  - `CONF_PKG_S3_COMPRESS`: Set this variable to 1 if you want to compress text files (raw text, HTML, JSON, SVG…) sent to Amzon S3. Files are compressed with GZIP. Note that these files are sent twice; once uncompressed, and once compressed (which overwrites the first one).
- **install rule**
